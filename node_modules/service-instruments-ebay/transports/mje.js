...
var Stream = require('streams-ebay'),
    MultipartStream = Stream.MultipartStream,
...
module.exports = function sseTransport(pipe, config) {
    ...
    pipe.on('request', function onRequest(request) {
        ...
        var eventhose = new MultipartStream();
        ...
        .on('data', function onstreamdata(eventdata) {
            ...
            if (eventdata) {
                if (eventdata && eventdata.type === 'ERROR') {
                    eventErr = eventdata;
                } else if (eventdata.id === 'end-event') {
                    this.emit('end');
                    return;
                }

                if (req.res.headers['content-type'] === 'multipart/json') {
                    eventdata = eventdata;
                }

                cal.createTransaction('Model', eventdata.id, function(
                    err, event) {
                    event.timestamp = st;
                    event.complete();
                });
