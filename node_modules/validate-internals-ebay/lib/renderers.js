...
const Labels = {
    ...
    JVM_MEM_USED: 'Java Process Memory Used',
    JVM_MEM_TOTAL: 'JVM Memory Total',
    JVM_MEM_AVAILABLE: 'JVM Memory Available',
...
module.exports.renderComponentStatus = function renderComponentStatus(req, res) {
    ...
    var templateName = 'component-status';

    if(componentStatus.isStatusAsync(component)) {
        componentStatus.getStatus(component, (statusModel) => {
            buildAndRender(statusModel, templateName, req, res);
        });
    } else {
        var statusModel = componentStatus.getStatus(component);
        return buildAndRender(statusModel, templateName, req, res);
    }
};

function buildAndRender(statusModel, templateName, req, res) {
    var component = req.query.component;
    ...
    if (!renderer) {
        renderXmlResult(templateName, model, req, res);
    } else {
    ...
    Async.times(ViUtils.getWorkersNumber(), (workerId, next) => {
        ...
        (err, res) => {
            ...
            var workerInfo = {
                ...
                status: info.status,
                // since it is not fast page, we can do array search
...
module.exports.renderDefaultComponentStatus = function renderDefaultComponentStatus(req, res) {
    ...
    function doRender(workerHostInfo) {
        ...
        // make memory usage look better
        const info = model.components[0];
        // since it is not fast page, we can do array search
        info.properties.forEach(prop => {
            if ([
                ...
                Labels.NODE_MEM_TOTAL,
                Labels.JVM_MEM_USED,
                Labels.JVM_MEM_AVAILABLE,
                Labels.JVM_MEM_TOTAL,
                Labels.PHYSICAL_MEM_AVAILABLE,
                Labels.PHYSICAL_MEM_TOTAL
            ].indexOf(prop.name) !== -1) {
                prop.value = Math.round(prop.value/1e6) + 'M';
            }
        });
        // now render
