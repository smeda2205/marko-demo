...
* @fnOnNewRowPosted			        Function    function(data) Function that can override the default function that is called when sAddURL returns the result
                                                    You can use this function to add different behaviour when server-side page returns result
* @fnOnAdded                    	Function    function(status){...} Function called after the add action. Status can be "success" or "failure", scope is (in case of "success") the TR of the newly-added row, or the form otherwise. 
    ...
    $.fn.makeEditable = function (options) {
        ...
        function _fnOnRowAdding(event) {
            if (properties.fnOnAdding()) {
                if (oAddNewRowForm.valid()) {
                    iDisplayStart = _fnGetDisplayStart();
                    properties.fnStartProcessingMode();
                    if ($.isFunction(properties.sAddURL)) {
                    	try {
                    		var data = properties.sAddURL.call(this, properties);
                    		_fnOnRowAdded(data);
                    	} catch (err) {
                            properties.fnEndProcessingMode();
                            properties.fnShowError(err.message, "add");
        ...
        function _fnOnRowAdded(data) {
            ...
            if (properties.fnOnNewRowPosted(data)) {
                ...
                _fnSetDisplayStart();
                properties.fnOnAdded("success", oTRAdded);
            }
        }
        ...
        function _fnDeleteRow(id, sDeleteURL) {
            ...
            var sFunctionResponse = undefined;
            if ($.isFunction(sURL)) {
            	try {
            		sFunctionResponse = sURL.call(this, data);
            		_fnOnRowDeleted(sFunctionResponse);
            	} catch (err) {
                    properties.fnEndProcessingMode();
                    properties.fnShowError(err.message, "delete");
