...
$.each($('#menu1 li'), function(i, v){
 var li = $(v);
 if (li.attr("id") == 'more') return;
 if (li.attr('id') == '1') return; // leave a first item
 rightOffset = li.offset().left; // rightOffset == 0 --> the menu item was out of screen
 
 if (rightOffset > 0) rightOffset = li.offset().left + li.width();
...
if(!more){
 ...
 setTimeout(function(){closeMenu('#moreMenu');}, 1500);
 });
...
setTimeout(function(){closeMenu('#moreLogsMenu');}, 1500);
});
