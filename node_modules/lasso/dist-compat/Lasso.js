...
Lasso.prototype = {
    ...
    lassoResource(data, options) {
        ...
        return _asyncToGenerator(function* () {
            ...
            if (!lassoContext) {
                lassoContext = options.lassoContext || _this5.createLassoContext(options);
            }
            ...
            if (resourceHandlers.hashPath || useCache) {
                cacheKey = resourceHandlers.calculateKey(data, _this5, lassoContext, options);
            }

            if (useCache) {
                const cache = _this5.getLassoCache(lassoContext);

                lassoResourceResult = yield cache.getLassoedResource(cacheKey, _asyncToGenerator(function* () {
                    return resourceHandlers.resource(_this5, data, cacheKey, options, lassoContext);
                }));
            } else {
                lassoResourceResult = yield resourceHandlers.resource(_this5, data, cacheKey, options, lassoContext);
            }
