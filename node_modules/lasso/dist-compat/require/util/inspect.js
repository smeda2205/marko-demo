...
function isRequire(node) {
    return node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'require' && node.arguments.length === 1 && node.arguments[0].type === 'Literal' && typeof node.arguments[0].value === 'string';
}

function isRequireResolve(node) {
    return node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && node.callee.object.type === 'Identifier' && node.callee.object.name === 'require' && node.callee.property.type === 'Identifier' && node.callee.property.name === 'resolve' && node.arguments.length === 1 && node.arguments[0].type === 'Literal';
}

function isRequireFor(node, moduleName) {
    return node.type === 'CallExpression' && node.callee.type === 'Identifier' && node.callee.name === 'require' && node.arguments.length === 1 && node.arguments[0].type === 'Literal' && node.arguments[0].value === moduleName;
}

function isRequireForAsyncLoader(node) {
    return isRequireFor(node, 'lasso-loader') || isRequireFor(node, 'raptor-loader');
}
...
function isAsyncNode(node, scope) {
    ...
    if (node.type !== 'CallExpression' || node.callee.type !== 'MemberExpression' || node.callee.property.type !== 'Identifier' || node.callee.property.name !== 'async') {
        return false;
    }
    ...
    if (node.callee.object.type === 'Identifier' && scope[node.callee.object.name] === 'lasso-loader') {
        return true;
    }
...
function parseAsyncNode(node, scope) {
    ...
    var hasFunctionBody = callbackNode.type === 'FunctionExpression' || callbackNode.type === 'FunctionDeclaration';
...
function recordGlobalsHelper(node, scope, foundGlobals) {
    ...
    if (id === 'require' || id === 'exports' || id === 'module' || id === 'arguments' || id === '__dirname' || id === '__filename') {
...
function recordGlobals(node, parentNode, scope, foundGlobals) {
    if (node.type === 'Identifier') {
        if (parentNode.type === 'MemberExpression' || parentNode.type === 'Property' || parentNode.type === 'VariableDeclarator' || parentNode.type === 'FunctionDeclaration' || parentNode.type === 'FunctionExpression') {
            return;
        }
