...
exports.parseOptions = function (options) {
    ...
    Object.keys(options.headers).forEach(function (name) {
        const headerVal = options.headers[name];
        if (Thing.isUndefined(headerVal)) {
            delete options.headers[name];
        }
        if (checkInvalidHeaderChar(headerVal)) {
            console.log(`Detected bad header "${name}":"${headerVal}", removing it`);
            delete options.headers[name];
        }
    });
    ...
    if (options.qs) {
        ...
        if (Thing.isObject(options.qs)) {
            ...
            options.qs = Querystring.stringify(Querystring.parse(options.qs));
        }
...
/**
 *   IMPORTED from https://github.com/nodejs/node/blob/v6.x/lib/_http_common.js
