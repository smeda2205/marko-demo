...
/**
 * For Serialization of Error object
 */
...
function formatTime(timestamp) {
    ...
    var ms = timestamp - 1000 * parseInt(timestamp/1000);
    // add a zero in front of numbers<10
    h = formatNumber(h);
    m = formatNumber(m);
    s = formatNumber(s);
    ms = String(formatMsNumber(ms)).substring(0, 2);
...
exports = module.exports = {
    ...
    format: function cal(args) {
        ...
        fields = [
            ...
            threadId,
            args.messageClass + formatTime(args.timestamp),
            args.type,
            args.name,
            args.status
        ];
        ...
        fields = fields.map(function (value) {
            return truncate(String(value), constants.MAX_FIELD_LENGTH);
        });
                    ...
                    if (key === 'msg' && value && value.length === 1 &&
                        ...
                        if (obj) {
                            if (value instanceof Error) {
                                datafield.push(querystring.unescape(escapeControlChars(querystring.stringify(obj))));
                                delete data[key];
