...
function send(cal) {
    ...
    setImmediate(function () {
        ...
        if (cal.eventName === 'complete') {
            cal.instance._flushed = true;
        }
    });
}
...
ArrayBuffer.prototype = {
    ...
    flushAll: function flushAll(threadId) {
...
function Buffer(threadId, flushDelayInterval) {
    this.threadId = threadId;
    this.children = [];
    this.map = {};
...
function getCalBuffer(threadId, tree, flushDelayInterval) {
    ...
    if (!buffer) {
        calBufferStore.set(threadId, tree ? new Buffer(threadId, flushDelayInterval) : new ArrayBuffer(threadId));
